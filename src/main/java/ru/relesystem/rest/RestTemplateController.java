...
import ru.relesystem.rest.model.RestPostsModel;
import ru.relesystem.rest.model.RestUserModel;
 ...
 */
@RestController // will add automatically the @ResponseBody annotation to all
				// methods
class RestTemplateController {

	/**
	 * Accessing a third-party REST service inside a Spring application it can
	 * even bind that data to custom domain types.
	 */
	@Autowired
	private RestTemplate restTemplate;

	private final String EXTERNAL_REST_URL = "http://jsonplaceholder.typicode.com"; // free
																					// JSON
																					// services
	...
	public List<RestUserModel> getRestUsers() {
		...
		ResponseEntity<RestUserModel[]> response = restTemplate.getForEntity(EXTERNAL_REST_URL + "/users",
				RestUserModel[].class);
	...
	public List<RestPostsModel> getRestPosts() {
		...
		ResponseEntity<RestPostsModel[]> response = restTemplate.getForEntity(EXTERNAL_REST_URL + "/posts",
	...
	public RestPostsModel getRestPostsById(@PathVariable("param") String param) {
		...
		ResponseEntity<RestPostsModel> response = restTemplate.getForEntity(EXTERNAL_REST_URL + "/posts/" + param,
				RestPostsModel.class);
	...
	public void deletePostByID(@PathVariable(value = "postId") String postId) {
		// in test case 100 posts. Try to del id 100+ (id=150 for example) and
